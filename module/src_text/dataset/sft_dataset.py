...
class SupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if len(SYSTEM_MESSAGE) > 0 and "Qwen3" not in self.model_id:
            ...
            system_message_input_ids = processor(system_message, add_special_tokens=False, return_tensors='pt')['input_ids']
        ...
        for _, j in enumerate(range(0, len(sources), 2)):
            ...
            if DEFAULT_IMAGE_TOKEN in user_input:
                ...
                prompt_input_ids = processor(user_input, add_special_tokens=False, padding=False, return_tensors='pt')['input_ids']

            response_input_ids = processor(gpt_response, add_special_tokens=False, padding=False, return_tensors='pt')['input_ids']
...
def get_pad_token_id(obj, default: int = 0) -> int:
    if hasattr(obj, "pad_token_id") and obj.pad_token_id is not None:
        return int(obj.pad_token_id)
    if hasattr(obj, "tokenizer") and hasattr(obj.tokenizer, "pad_token_id") and obj.tokenizer.pad_token_id is not None:
        return int(obj.tokenizer.pad_token_id)
    return default
...
def make_supervised_data_module(model_id, processor, data_args):
    ...
    pad_id = get_pad_token_id(processor, default=0)
    data_collator = DataCollatorForSupervisedDataset(pad_token_id=pad_id)
